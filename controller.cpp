...
void Controller::importXMLFile(QString file_path)
{
    ...
    if (type == "itrace_core")
    {
        importCoreXML(file_path);
    }
    else if (type == "itrace_plugin")
    {
        importPluginXML(file_path);
    }
    else
    {
...
void Controller::batchAddXML(QString folder_path)
{
    ...
    while (dir.hasNext())
    {
        ...
        if (type == "itrace_core" || type == "itrace_plugin")
        {
            ...
            {
                (files.find(id))->second.first.push_back(filename);
            }
            if (type == "itrace_core")
            {
                files.find(id)->second.second = true;
            }
        }
        //}
    ...
    for (auto i = files.begin(); i != files.end(); ++i)
    {
        if (i->second.first.size() >= 2 && i->second.second)
        {
            ...
            {
                if (!idb.fileExists(QCryptographicHash::hash(j.toUtf8().constData(), QCryptographicHash::Sha1).toHex()))
                {
                    importXMLFile(j);
                }
                else
    ...
    if (badPairWarn != "")
    {
        warn += "The following files were missing their associated pair file:\n" + badPairWarn;
    }
    if (alreadyInWarn != "")
    {
        warn += "The following files are already imported:\n" + alreadyInWarn;
    }
    if (warn != "")
    {
...
void Controller::generateFixationData(QVector<QString> tasks, QString algSettings)
{
    ...
    for (auto i : tasks)
    { // Get the sessions that the user wants to use
    ...
    for (auto session_id : sessions)
    {
        ...
        std::sort(session_fixations.begin(), session_fixations.end(), [](const Fixation &a, const Fixation &b) -> bool
                  { return a.fixation_event_time > b.fixation_event_time; });
        ...
        for (auto fix = session_fixations.rbegin(); fix != session_fixations.rend(); ++fix)
        {
            ...
            {
                if (unique_gazes.find(gaze.event_time) != unique_gazes.end())
                {
                    continue;
                }
...
// std::map<QString, double> SRCML_CATEGORY_WEIGHTS = {
//     // developer defined, frequent use/high semantic relevance
//     {"complex-name-expression": 0.30},
//     {"literal": 0.30},
//     {"function-call": 0.30},
//     {"function-name": 0.30},
//     {"name-expression": 0.30},

//     // may be developer defined, high familiarity, moderate to low semantic relevance
//     {"type-name": 0.15},
//     {"name-declaration": 0.15},
//     {"operator": 0.15},
//     {"constructor-name": 0.15},
//     {"keyword": 0.15},
//     {"class-name": 0.15},

//     // not developer defined, little to no semantic relevance
//     {"function-type-specifier": 0},
//     {"constructor-specifier": 0},
//     {"type-specifier": 0},
//     {"whitespace": 0},
//     {"seperator": 0},
//     {"class-specifier": 0},
...
void Controller::correctFixationData(QVector<QString> tasks, QString algSettings){
...
QString Controller::findMatchingPath(QVector<QString> all_files, QString file)
{
    ...
    {
        if (i.toLower().endsWith(check))
        {
            possible.push_back(i.split("/"));
        }
    }
    if (possible.size() == 0)
    {
