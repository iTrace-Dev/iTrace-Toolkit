...
void Database::insertCalibrationPoint(QString calibration_point_id, QString calibration_id, QString calibration_x, QString calibration_y) {
    db.exec(QString("INSERT INTO calibration_point(calibration_point_id,calibration_id,calibration_x,calibration_y) VALUES(\"%1\",%2,%3,%4)").arg(calibration_point_id,calibration_id,calibration_x,calibration_y));
}
...
void Database::insertCalibrationSample(QString calibration_point_id, QString left_x, QString left_y, QString left_validation, QString right_x, QString right_y, QString right_validation) {
    db.exec(QString("INSERT INTO calibration_sample(calibration_point_id,left_x,left_y,left_validation,right_x,right_y,right_validation) VALUES(\"%1\",%2,%3,%4,%5,%6,%7)").arg(calibration_point_id,left_x,left_y,left_validation,right_x,right_y,right_validation));
}

void Database::insertFile(QString file_hash, QString session_id, QString file_full_path, QString file_type) {
    db.exec(QString("INSERT INTO files(file_hash,session_id,file_full_path,file_type) VALUES(\"%1\",%2,\"%3\",\"%4\")").arg(file_hash,session_id,file_full_path,file_type));
}
...
void Database::insertFixationGaze(QString fixation_id, QString event_time) {
    db.exec(QString("INSERT INTO fixation_gaze(fixation_id,event_time) VALUES(\"%1\",%2)").arg(fixation_id,event_time));
}

void Database::insertFixationRun(QString fixation_run_id, QString session_id, QString date_time, QString filter) {
    db.exec(QString("INSERT INTO fixation_run(fixation_run_id,session_id,date_time,filter) VALUES(%1,%2,%3,\"%4\")").arg(fixation_run_id,session_id,date_time,filter));
}
...
void Database::insertParticipant(QString participant_id, QString session_length) {
    db.exec(QString("INSERT INTO participant(participant_id,session_length) VALUES(\"%1\",%2)").arg(participant_id,session_length));
}
...
void Database::insertWebContext(QString event_time, QString browser_type, QString site_name, QString url, QString tag) {
    db.exec(QString("INSERT INTO web_context(event_time,browser_type,site_name,url,tag) VALUES(%1,\"%2\",\"%3\",\"%4\",\"%5\")").arg(event_time,browser_type,site_name,url,tag));
}

QString Database::getSessionFromParticipantAndTask(QString participant_id, QString task_name) {
    QSqlQuery session_id = db.exec(QString("SELECT session_id FROM session WHERE participant_id = \"%1\" AND task_name = \"%2\"").arg(participant_id,task_name));
    session_id.first();
...
QVector<QString> Database::getAllIDEContextIDs() {
    QVector<QString> data;
    QSqlQuery ids = db.exec("SELECT event_time FROM ide_context");
    while(ids.next()) {
        data.push_back(ids.value(0).toString());
    }
    return data;
}
...
QString Database::getUpdateGazeWithSyntacticInfoQuery(QString event_id, QString xpath, QString syntactic_context) {
    xpath.replace("\"","'");
...
QString Database::getUpdateGazeWithTokenInfoQuery(QString event_id, QString token, QString token_type) {
...
void Database::executeLongQuery(QString big_query) {
    db.exec(big_query);
}
